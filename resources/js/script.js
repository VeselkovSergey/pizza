                ...
                '<div class="container-product-in-basket w-100 ">' +
                    ...
                    '<div class="flex-space-between">' +
                        ...
                        '<div class="buttons-edit-amount-product border-radius-25 flex-center" style="background-color: rgb(243, 243, 247);">' +
                            ...
                            '<div class="amount-product flex-center color-black" style="min-width: 20px; min-height: 20px; margin: 5px 5px 3px 5px;" data-modification-id="' + modificationId + '">' + amount + '</div>' +
...
function OrderInfoGenerationHTML(orderId) {
    ...
    if (countProductsInBasket !== 0) {
        return  '<div class="client-information w-100">' +
                    ...
                    '<div class="w-100 flex-center mt-25"><button class="cp order-create btn first">' + (orderId ? 'Сохранить изменения' : (auth ? 'Оформить заказ' : 'Авторизоваться')) + '</button>' + (orderId ? '<button class="cp clean-basket btn first mt-25 ml-10">Очистить данные</button>'  : '') + '</div>' +
                '</div>';
    } else {
        return '';
    }
