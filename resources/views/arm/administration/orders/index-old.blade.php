...
@section('js')
    ...
    <script>
        ...
        changeDateFields.forEach((changeDateField) => {
            changeDateField.addEventListener('change', () => {
                ...
                if (startDate && endDate) {
                    location.href = "{{route('administrator-arm-orders-old-page')}}?start-date=" + startDate + "&end-date=" + endDate;
                }
            });
        });
        ...
        allOrdersTodayButton.addEventListener('click', () => {
            LoaderShow();
            location.href = "{{route('administrator-arm-orders-old-page')}}??start-date={{date('Y-m-d', time())}}&end-date={{date('Y-m-d', time())}}";
        });

        let allOrdersButton = document.body.querySelector('.all-orders');
        allOrdersButton.addEventListener('click', () => {
            LoaderShow();
            location.href = "{{route('administrator-arm-orders-old-page')}}?all=true";
        });
