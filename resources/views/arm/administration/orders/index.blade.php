...
@section('content')
    ...
    <div class="flex-column">
        ...
        <div style="order: 1;">
            ...
            <div class="mb-10">
                <div class="toggle-button cp" data-toogle="amount-orders-in-days-container">Кол-во заказов в день</div>
                <div class="amount-orders-in-days-container">
                    @foreach($amountOrdersInDays as $date => $amountOrdersInDay)
                        <div>Дата: {{date('Y-m-d', strtotime($date))}} - {{$amountOrdersInDay['cash']}}/{{$amountOrdersInDay['bank']}} (нал/банк) (сумма: {{$sumOrdersInDays[$date]['cash']}}/{{$sumOrdersInDays[$date]['bank']}} (нал/банк))</div>
                    @endforeach
                </div>
            </div>
        </div>
